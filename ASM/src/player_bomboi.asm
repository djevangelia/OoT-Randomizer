Player_DetachActorCheckBomb:
    addiu   sp,sp,-24
    sw      ra,16(sp)
    lw      v1,924(s0)          ; displaced
    lb      at,CFG_BOMB_OI      ; check if bomb OI enabled
    bnez    at,@@SetBombJump
    lui     t9,0x8083
    b       @@CheckConditions
    addi    t9,0x03f8           ; address at function return
@@SetBombJump:
    addi    t9,0x03d4           ; address for explosives check
@@CheckConditions:
    ...
    beqz    v1,@@SetBranch      ; no held actor
    ...
    lw      v1,60(sp)
    b       @@Return
    move    v0,zero             ; don't branch, run set null instructions
@@SetBranch:
    li      v0,1                ; branch to explosives check or return
@@Return:
    lw      ra,16(sp)
    jr      ra
    addiu   sp,sp,24
